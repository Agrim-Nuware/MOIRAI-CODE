...
CUTOFF_DATE (2024-10-01 by default -- see prepare_oct2024_split_data.py),
using only the context variates the model was fine-tuned on: Close, Return,
LogReturn. Open/High/Low/Volume/Day/Month/Momentum/MovingAverage are never
fed to the model -- they were excluded from context at data-prep time.

In addition to the 3 context variates' own forecasts, this also reconstructs
an implied Close price path from the forecasted LogReturn path (anchored at
the last known Close in each window's context) and compares it against both
the actual Close path and the model's direct Close-channel forecast. This is
the practical way to check whether forecasting in (near-stationary) return
space produces a better price forecast than forecasting the (non-stationary)
raw Close level directly.

Produces:
  - dataset/sp500/results_v2_metrics.json
  - dataset/sp500/results_v2_forecast_plot.png    actual vs zero-shot vs fine-tuned vs reconstructed (Close)
  - dataset/sp500/results_v2_metrics_bar.png       Close MAPE + Return/LogReturn MAE, zero-shot vs fine-tuned
  - dataset/sp500/results_v2_loss_curve.png        train/val loss curve (if a TensorBoard log is found)

Usage:
  python dataset/sp500/evaluate_finetuned_v2.py --checkpoint <path/to/*.ckpt>
"""
...
VARIATES = ["Close", "Return", "LogReturn"]
...
parser.add_argument("--full_csv", default=f"{OUT_DIR}/sp500_full_features.csv")
parser.add_argument("--split_info", default=f"{OUT_DIR}/split_info_v2.json")
...
df = pd.read_csv(args.full_csv, index_col=0, parse_dates=True)[VARIATES]
...
logreturn_idx = VARIATES.index("LogReturn")
...
def compute_metrics(forecasts, labels, inputs):
    per_variate = {v: {"mae": []} for v in VARIATES}
...
with open(f"{OUT_DIR}/results_v2_metrics.json", "w") as f:
...
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/results_v2_forecast_plot.png", dpi=150)
print(f"\nSaved {OUT_DIR}/results_v2_forecast_plot.png")
...
plt.tight_layout()
plt.savefig(f"{OUT_DIR}/results_v2_metrics_bar.png", dpi=150)
print(f"Saved {OUT_DIR}/results_v2_metrics_bar.png")
...
try:
    ...
    if tb_dirs:
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(f"{OUT_DIR}/results_v2_loss_curve.png", dpi=150)
        print(f"Saved {OUT_DIR}/results_v2_loss_curve.png")
    else:
